...
from app.services import create_incident, get_active_incidents, get_history, resolve_incident
...
async def create_incident_endpoint(payload: IncidentCreate, db: Session = Depends(get_db)):
    incident = create_incident(db, payload.message)
...
def get_incidents_history(
    ...
    items, total = get_history(db, status, from_, to, limit, offset)
